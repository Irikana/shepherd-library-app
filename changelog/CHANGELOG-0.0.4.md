...
- **站内搜索数据同步**：上传的文章/新闻自动写入网站 `js/library-dynamic.js` 的站内搜索数据（Search.data），从此 SlyWrite 上传的内容可通过网站右上角搜索（查找按钮）按标题或关键词找到
- **隐藏文章选项**：元数据表单新增「隐藏文章」开关；隐藏后不同步 library.html 与新闻区等公开列表（仍上传文章文件），但照常加入站内搜索数据——只能通过查找按钮找到
